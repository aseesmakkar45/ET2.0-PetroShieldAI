...
Originally developed during a hackathon, the project has evolved into a robust React + FastAPI architecture intended for national security applications, logistics research, and open-source collaboration.
...
- Automatic triggering of the 5-Agent Cascade
...
<p align="center">
  <img src="./assets/map-view.png" width="90%">
</p>
A live interactive Leaflet map displaying global chokepoints, active vessel routes, and real-time maritime weather data fetched from Open-Meteo.

---

## 🛒 Procurement Orchestrator
<p align="center">
  <img src="./assets/procurement.png" width="90%">
</p>
The SciPy-powered optimization dashboard showing ranked alternatives for cargo rerouting (e.g., Russian Urals vs. Saudi Yanbu Pipeline) complete with freight discounts and transit duration.

---

## 📊 Supply Chain Digital Twin
<p align="center">
  <img src="./assets/twin.png" width="90%">
</p>
A node-graph visually modeling the downstream impact of a disruption from a global supplier down to a domestic Indian refinery.
...
The core innovation of PetroShield AI is its 5-stage autonomous pipeline. The following sequence illustrates exactly what happens internally from the moment a global event occurs to the moment a decision is presented on the dashboard.

## 1. The Geopolitical Trigger (Autonomous Brain)

The system does not wait for a user to type a query. 
A background asyncio loop in FastAPI polls the GDELT Project database every few minutes for global news related to energy, conflict, and logistics.
...
- It assigns a numerical Risk Score (1-10).
- If the score exceeds the critical threshold (>= 7), the Autonomous Brain fires a system-wide alert and initiates the Agent Cascade.

---

## 2. Risk Intelligence Extraction (Agent 1)

The Risk Intel Agent receives the raw news payload.
Unlike standard chatbots, its prompt forces it to output strict JSON containing specific operational parameters:
- **Impacted Chokepoint**: (e.g., `bab_el_mandeb`)
- **Severity**: (e.g., `CRITICAL`)
- **Volume Disrupted**: (e.g., `1.2 mbpd`)
- **Primary Downstream Victim**: (e.g., `Sikka Port`)

These structured parameters are the crucial bridge between natural language text and deterministic mathematics.

---

## 3. Financial & Operational Scenario Modeling (Agent 2)

The Scenario Modeler receives the disrupted volume.
...
- It simulates 10,000 possible future price paths over a 30-day horizon.
...
## 6. Executive Briefing Generation (Agent 5)

Raw matrices and probability distributions are useless to politicians in a crisis.
The final agent ingests the outputs of Agents 1-4 and acts as a senior speechwriter. It produces a concise, highly-structured 3-sentence brief summarizing the threat, the financial impact, and the recommended operational response.
...
- The Procurement Orchestrator populates the top 2 mathematical reroute options.
- The Executive Brief appears on the main dashboard.

All of this happens within **5 to 10 seconds** of the news being published.
...
- **Why Groq?** Speed. The 5-agent cascade requires sequential LLM calls. A traditional LLM API would take 30-60 seconds for 5 consecutive calls. Using Groq's LPU inference for Llama 3 70B reduces the pipeline execution time to roughly ~5 seconds.
- **Free-Tier Resilience:** We intentionally opted for Open-Meteo (marine weather) and GDELT (news) to ensure the platform requires zero paid API keys to run its core monitoring features.
...
| **AI Inference** | Groq API (Llama-3-70b-versatile, Llama-3-8b-instant) |
...
- `backend/agents/`: The autonomous cognitive engines (Risk Intel, Scenarios, Procurement, SPR, Executive, Brain).
- `backend/services/`: External API integrations (GDELT, Open-Meteo).
...
- `frontend/components/`: Modular React components (CommandCenter, Maps, Charts).
...
# Optional: For frontend CORS if running on a custom domain (defaults to localhost & vercel)
...
- **WebSocket Chunking:** Rather than sending massive JSON payloads, the WebSocket broadcaster sends localized state updates to minimize rendering overhead on the React client.
